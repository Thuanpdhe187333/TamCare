    ...
    if ("POST".equalsIgnoreCase(request.getMethod())) {
        ...
        if (linkKey == null || linkKey.trim().isEmpty()) {
            ...
            Integer elderlyId = dao.getElderlyIdByLinkKey(linkKey.trim());
                ...
                msg = ok ? "✅ Liên kết thành công!" : "❌ Liên kết thất bại!";
}
        }
    }

    // Load danh sách người thân theo Relationship
    UserDAO dao = new UserDAO();
    ...
    <body>      
        <aside class="sidebar">
            ...
            <nav class="sidebar-menu">
                ...
                <a href="my-relatives" class="menu-link active"><i class="fa-solid fa-users"></i> Người thân của tôi</a>
        ...
        <main class="main-content">
            ...
            <%
        String err = (String) session.getAttribute("link_error");
        String ok  = (String) session.getAttribute("link_success");
            %>

            <form action="link-elderly" method="post" style="margin: 10px 0 25px 0; display:flex; gap:10px; align-items:center;">
                <input name="linkKey" placeholder="Nhập mã kết nối (VD: TC123456)" required
                       style="flex:1; padding:12px 14px; border-radius:12px; border:1px solid #e2e8f0; outline:none;">
                <button type="submit" style="padding:12px 18px; border:none; border-radius:12px; background:var(--primary); color:#fff; font-weight:600; cursor:pointer;">
                    Liên kết
                </button>
            </form>

            <% if (err != null) { %>
            <p style="color:#e11d48; font-weight:600; margin:0 0 12px 0;"><%= err %></p>
            <% session.removeAttribute("link_error"); %>
            <% } %>

            <% if (ok != null) { %>
            <p style="color:#16a34a; font-weight:600; margin:0 0 12px 0;"><%= ok %></p>
